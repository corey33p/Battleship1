package com.example.battleship1;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bitmap _youWinImg = drawableToBitmap(getResources().getDrawable(R.drawable.you_win));
        this.youWinImg = Bitmap.createScaledBitmap(_youWinImg, cellSize, cellSize, true);
        Bitmap _aiWinsImg = drawableToBitmap(getResources().getDrawable(R.drawable.ship));
        this.aiWinsImg = Bitmap.createScaledBitmap(_aiWinsImg, cellSize, cellSize, true);
        resetGame();
        ...
        if (this.placingShips){
            if ((event.getAction() == MotionEvent.ACTION_DOWN) || (event.getAction() == MotionEvent.ACTION_MOVE)) {
                ...
                if (validLocations(locationsCheck,true)){
                    ...
                    this.validTentativeShipLocation = false;
...
//    }
    ...
    private void changeTurn(){
        ...
        if (!playerOnesTurn){
            ...
            },2000); // delay should be at least 1000 due to previous delay
        }
    ...
    private void setWinImg(boolean youWin){
        Rect imgBounds = new Rect(0,0,getScreenWidth(),getScreenWidth());
        // update both player one and player two images with
        Bitmap result = Bitmap.createBitmap(getScreenWidth(), getScreenWidth(), playerOneGrid.getConfig());
        Canvas canvas = new Canvas(result);
        if (youWin){
            canvas.drawBitmap(playerOneGrid, null, imgBounds, null);
            canvas.drawBitmap(youWinImg, null, imgBounds, null);
            playerOneGrid = result;
            //
            result = Bitmap.createBitmap(getScreenWidth(), getScreenWidth(), playerTwoGrid.getConfig());
            canvas = new Canvas(result);
            canvas.drawBitmap(playerTwoGrid, null, imgBounds, null);
            canvas.drawBitmap(youWinImg, null, imgBounds, null);
            playerTwoGrid = result;
        } else {
            canvas.drawBitmap(playerOneGrid, null, imgBounds, null);
            canvas.drawBitmap(aiWinsImg, null, imgBounds, null);
            playerOneGrid = result;
            //
            result = Bitmap.createBitmap(getScreenWidth(), getScreenWidth(), playerTwoGrid.getConfig());
            canvas = new Canvas(result);
